...
def plotter(mnist_data: List[List[float]]):
    train_size = [2000, 4000, 6000, 8000, 10000, '']
    epoch_size = [10, 20, 30, 40, 50, '']
    plt.imshow(mnist_data[::-1], extent=[0, len(mnist_data[0]), 0, len(mnist_data)])
    plt.colorbar(label="Accuracy of alg.")
    plt.xlabel("Epochs")
    plt.ylabel("Test size")
    plt.xticks(np.arange(0, len(mnist_data[0])+1, 1), labels=epoch_size)
    plt.yticks(np.arange(0, len(mnist_data)+1, 1), labels=train_size)
    plt.show()
    ...
    if not arguments['layers']:
        if arguments['train_size'] or arguments['epochs']:
            ...
            with tqdm(total=len(train_size)*len(epoch_size)) as progress_bar:
                for i, ts in enumerate(train_size):  # 1000, 2000, 5000, 10000, 30000
                    mnist_results.append([])
                    for ep in epoch_size:
                        network = create_network([3000, 1000], x_train, y_train, sigmoid, sigmoid_prime,
